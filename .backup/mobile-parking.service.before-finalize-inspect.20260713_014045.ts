...
export class MobileParkingService {
  ...
  async finalizeInvoiceForSessionId(sessionId: string, options?: { exitTime?: Date }) {
    ...
    const previousUnpaidAmount = session.invoice?.unpaidAmount ?? 0;
    const existingInvoiceWasFullyPaid =
      Boolean(session.invoice) &&
      (session.invoice?.status === 'PAID' ||
        (paidAmount > 0 && previousUnpaidAmount <= 0));
    const unpaidAmount = Math.max(0, finalAmount - paidAmount);
    const isAdditionalFeeInvoice =
      existingInvoiceWasFullyPaid && unpaidAmount > 0;
    ...
    const invoicePaidAt =
      unpaidAmount <= 0
        ? (session.invoice?.paidAt ?? new Date())
        : paidAmount > 0
          ? (session.invoice?.paidAt ?? null)
          : null;
