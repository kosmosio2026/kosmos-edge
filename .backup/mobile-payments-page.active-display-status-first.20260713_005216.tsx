...
function withCurrentSessionId(path: string, fallbackSessionId = '') {
  const sessionId = getCurrentParkingSessionId() || fallbackSessionId;
...
function getCurrentParkingAuthToken(sessionIdOverride = '') {
  if (typeof window === 'undefined') return '';
  ...
  const sessionId = getCurrentParkingSessionId() || sessionIdOverride;
...
function getPaymentSessionId(item: any) {
  return (
    item?.sessionId ??
    item?.parkingSessionId ??
    item?.parkingSession?.id ??
    item?.invoice?.sessionId ??
    item?.session?.id ??
    item?.parkingSessionId ??
    item?.id ??
    ''
...
function getFinalizeSessionId(items: any[]) {
  const storedSessionId = getCurrentParkingSessionId();

  if (storedSessionId) return storedSessionId;

  const activeUnpaid = items.find((item) => {
    const status = item?.status ?? item?.sessionStatus;
    const unpaidAmount = Number(item?.invoice?.unpaidAmount ?? 0);

    return (
      ['ACTIVE', 'GRACE_PERIOD'].includes(String(status)) &&
      unpaidAmount > 0 &&
      Boolean(getPaymentSessionId(item))
    );
  });

  if (activeUnpaid) return String(getPaymentSessionId(activeUnpaid));

  const unpaid = items.find((item) => {
    const invoiceStatus = item?.invoice?.status;
    const unpaidAmount = Number(item?.invoice?.unpaidAmount ?? 0);

    return (
      !['PAID', 'VOID', 'CANCELED'].includes(String(invoiceStatus)) &&
      unpaidAmount > 0 &&
      Boolean(getPaymentSessionId(item))
    );
  });

  if (unpaid) return String(getPaymentSessionId(unpaid));

  const anyCurrent = items.find((item) => Boolean(getPaymentSessionId(item)));

  return anyCurrent ? String(getPaymentSessionId(anyCurrent)) : '';
}

function isActiveParkingItem(item: any) {
  const status = String(
    item?.status ?? item?.sessionStatus ?? item?.parkingSession?.status ?? '',
  ).toUpperCase();

  return ['ACTIVE', 'GRACE_PERIOD'].includes(status) && !item?.exitTime;
}

function formatExitDisplay(item: any) {
  if (isActiveParkingItem(item)) return '주차 중';
  return formatDate(item?.exitTime);
}
...
export default function MobilePaymentsPage() {
  ...
  async function finalizeCurrentInvoice() {
    const finalizeSessionId = getFinalizeSessionId(items);
    const token = getCurrentParkingAuthToken(finalizeSessionId);
    ...
    if (finalizeSessionId && typeof window !== 'undefined') {
      localStorage.setItem('kosmos.currentParkingSessionId', finalizeSessionId);
      if (!localStorage.getItem('kosmos.currentParkingSessionOwnerType')) {
    ...
    try {
      const res = await fetch(`${API_BASE}${withCurrentSessionId('/mobile/parking/current/finalize-invoice', finalizeSessionId)}`, {
        method: 'POST',
        headers: {
          authorization: `Bearer ${token}`,
        },
