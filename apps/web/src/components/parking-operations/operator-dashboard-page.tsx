...
function gridHref(lotId: string | null | undefined, mode: 'manual' | 'sensor', target?: string) {
  ...
  return `/operator/grid?${params.toString()}`;
}
...
function SensorParkingLotCards({ group }: { group: ParkingLotGroup }) {
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-6">
        ...
        <StatCard
          title="빈 주차면"
          value={empty}
          href={gridHref(group.id, 'sensor', 'empty')}
          description="센서 기준 빈면"
        />
        <StatCard
          title="입차 중"
          value={occupied}
          href={gridHref(group.id, 'sensor', 'occupied')}
          description="센서 점유"
          tone="blue"
        />
        <StatCard
          title="등록 필요"
          value={registration}
          href={gridHref(group.id, 'sensor', 'registration')}
        ...
        <StatCard
          title="결제/청구"
          value={payment}
          href={gridHref(group.id, 'sensor', 'payment')}
        ...
        <StatCard
          title="센서 오류"
          value={sensorError}
          href={gridHref(group.id, 'sensor', 'sensor-error')}
          description="센서 점검 필요"
...
function ManualParkingLotCards({ group }: { group: ParkingLotGroup }) {
  const total = group.spaces.length;
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-3">
        ...
        <StatCard
          ...
          href={gridHref(group.id, 'manual', 'empty')}
          description="빈 주차면"
          tone="emerald"
        />
        <StatCard
          title="입차 중"
          value={entryActive}
          href={gridHref(group.id, 'manual', 'manual-active')}
    ...
    <main className="mx-auto flex max-w-7xl flex-col gap-6 p-6">
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-sm font-bold text-blue-600">Operator Home</p>
      ...
      {groups.length === 0 ? (
        ...
        groups.map((group) =>
          group.operationMode === 'MANUAL' ? (
            <ManualParkingLotCards key={group.id} group={group} />
          ) : (
            <SensorParkingLotCards key={group.id} group={group} />
          ),
        )
      )}
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              입차 중인 주차면을 우선 표시합니다. 출차 처리가 필요한 건을 확인하세요.
            </p>
          </div>
          <Link
            href="/operator/grid?target=alerts"
            className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-black text-white"
          >
            전체 보기
          </Link>
          ...
          <table className="min-w-full divide-y divide-slate-200 text-sm">
            <thead className="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
              <tr>
                ...
                <th className="px-4 py-3">출차 처리</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 bg-white">
              {actionSpaces.length === 0 ? (
                ...
                actionSpaces.map((space, index) => {
                  ...
                  const lotId = space.parkingLotId ?? null;
                  const href = isManualLot(space)
                    ? gridHref(lotId, 'manual', 'manual-active')
                    : gridHref(
                        lotId,
                        'sensor',
