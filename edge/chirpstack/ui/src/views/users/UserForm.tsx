...
function UserForm(props: IProps) {
  ...
  return (
    ...
    >
      <Form.Item label="이메일" name="email" rules={[{ required: true, message: "이메일 주소를 입력해 주세요!" }]}>
        <Input />
      </Form.Item>
      ...
      {props.password && (
        <Form.Item label="Password" name="password" rules={[{ required: true, message: "암호를 입력해 주세요!" }]}>
          <Input type="password" />
        </Form.Item>
      )}
      <Row>
